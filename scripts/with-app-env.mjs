...
function main(argv) {
  ...
  // Invoke Vite's JavaScript entrypoint directly so the wrapper works on
  // Windows too, where npm exposes the executable as a `.cmd` shim.
  const executable = command === "vite" ? process.execPath : command;
  const executableArgs = command === "vite"
