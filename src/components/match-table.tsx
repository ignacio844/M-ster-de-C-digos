...
function CodeOnly({ match, label }: { match: DisplayMatch; label: string }) {
  return (
    <div className="min-w-0" data-source={label} aria-label={`${label}: ${match.code ?? "sin código"}`}>
      <p
        className="truncate font-mono text-xs font-medium text-fg"
        title={match.code ?? "Sin código"}
      >
        {match.code ?? "—"}
      </p>
    </div>
                ...
                {pageRows.map(({ row, matches }) => (
                  <tr key={row.bam.id} className="border-t border-border align-top">
                    {matches.map((match, index) => (
                      ...
                      >
                        {index === 0 ? (
                          <CodeOnly match={match} label={MASTER_COLUMNS[index].label} />
                        ) : (
            ...
            {pageRows.map(({ row, matches }) => (
              <li key={row.bam.id} className="p-4">
                <div className="rounded-lg bg-chip/70 p-3">
                  ...
                  <CodeOnly match={matches[0]} label="BAMINDS" />
                </div>
